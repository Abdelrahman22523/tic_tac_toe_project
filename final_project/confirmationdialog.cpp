...
ConfirmationDialog::ConfirmationDialog(QWidget *parent) : QDialog(parent) {
    ...
    buttonback->setStyleSheet("QPushButton { background-color: #FFA500; color: black; font-size: 14px; padding: 6px; }"); // Orange button style
    buttonback->setMaximumWidth(200);
    connect(buttonback, &QPushButton::clicked, this, &ConfirmationDialog::back_clicked);
...
void ConfirmationDialog::back_clicked() {
    emit gameModeSelected(2); // 1 vs AI mode selected
    close();
